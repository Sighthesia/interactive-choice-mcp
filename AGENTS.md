...
- Tool contract: schema-first `provide_choice` with `title`, `prompt`, `type (single_select|multi_select|text_input|hybrid)`, `options[id,description]`, `allow_cancel`, `placeholder`. Output must include `action_status (selected|custom_input|cancelled|timeout)` and normalized selections.
