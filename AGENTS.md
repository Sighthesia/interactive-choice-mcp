...
- Tool contract: schema-first `provide_choice` with `title`, `prompt`, `selection_mode (single|multi|text_input|hybrid)`, `options[id,description]`, `placeholder`. Output must include `action_status (selected|custom_input|cancelled|timeout)` and normalized selections.
