...
- **Transport selection**: Orchestrator builds defaults from persisted config and chooses terminal hand-off when config.transport == "terminal" (returns `pending_terminal_launch`) or web otherwise; `session_id` short-circuits to poll existing terminal/web session.
...
- **Terminal UI defaults**: questionary-based flows in [../src/terminal/runner.py](../src/terminal/runner.py) and [../src/terminal/ui.py](../src/terminal/ui.py) support annotations, defaulting recommended options when `use_default_option` is true; settings prompt supports language, transport (if allowed), timeout, timeout defaults/actions.
- **Web flow**: FastAPI server in [../src/web/server.py](../src/web/server.py) creates ChoiceSessions, opens the browser, streams status via `/ws/{id}` and `/ws/interactions`, and persists results; switching from terminal sets transport `terminal-web` and reuses same session.
...
- **Configuration persistence**: User settings stored at `.mcp-data/config.json` (or `MCP_DATA_DIR`) through [../src/infra/storage.py](../src/infra/storage.py); env `CHOICE_TIMEOUT_SECONDS` can override request timeout; `ConfigStore.save(..., exclude_transport=True)` preserves transport during terminal→web switch.
