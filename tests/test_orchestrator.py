...
def test_orchestrator_prefers_terminal_when_available(monkeypatch):
    orch = ChoiceOrchestrator()
...
def test_orchestrator_falls_back_to_web(monkeypatch):
    orch = ChoiceOrchestrator()
    monkeypatch.setattr("choice.orchestrator.is_terminal_available", lambda: False)

    async def fake_web(req, timeout_seconds):
        return models.normalize_response(
            req=req,
            selected_ids=["b"],
            custom_input=None,
