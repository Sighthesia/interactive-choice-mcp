## Context
Users need an explicit configuration surface in both terminal and web flows to choose transport, adjust which options are presented, and tune timeout before answering. Current flows assume defaults and only present the choice prompt.

## Goals / Non-Goals
- Goals: expose transport selection, option visibility, and timeout settings ahead of both terminal and web prompts; feed selections into orchestrator and result payloads; keep defaults simple.
...
- Persist last-used configuration (transport, option visibility, timeout) in a lightweight store (e.g., file or in-memory cache keyed to session) and prefill subsequent prompts with those values, while still allowing overrides.
