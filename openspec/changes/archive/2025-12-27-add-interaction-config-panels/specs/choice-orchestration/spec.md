...
- **WHEN** terminal interface is selected and the configuration surface is shown
- **THEN** the user can accept defaults or adjust interface, option visibility (arbitrary toggles), and timeout, and the subsequent terminal prompt uses those selections while keeping the UI clear-and-summary behavior.

#### Scenario: Web configuration applied
- **WHEN** web interface is selected or required and the configuration panel is shown in the portal
- **THEN** the panel exposes interface choice (when applicable), option visibility, and timeout controls, and the ensuing web prompt honors the selections and tears down after returning the result payload.

#### Scenario: Persisted configuration reused
- **WHEN** a subsequent invocation starts and a saved configuration exists
- **THEN** the configuration surface pre-populates interface, option visibility, and timeout from the saved values while still allowing overrides, and updates the stored profile after completion.
