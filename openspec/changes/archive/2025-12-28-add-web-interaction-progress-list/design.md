## Context
- The web portal currently lists active interactions but lacks visibility into status transitions (submitted, auto-submitted, timeout, pending) and interface type (web vs terminal hand-off), which makes concurrent sessions ambiguous.
...
## Decisions
- Maintain a server-side interaction registry that tracks session id, interface type, created/updated timestamps, and status transitions (pending, submitted, auto-submitted, cancelled, timeout) for both web and terminal transports.
...
- Do we need user-facing filters beyond active vs completed (e.g., interface-specific, status-specific) in the initial version?
