...
- **i18n/config centralization**: `i18n.js` exposes `t(key)` using the injected JSON, and `config.js` owns language/theme/notification/transport persistence plus debounce logic. Other modules consume these helpers instead of touching DOM or storage directly.
